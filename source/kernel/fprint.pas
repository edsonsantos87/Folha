...
function kPrintcxGrid( tv: TcxGridDBTableView; Titulo: String;
...
begin
  ...
  for i := 0 to tv.ColumnCount -1 do
    tv.Columns[i].Tag := 0;  // Nao imprimir

  for i := 0 to ( tv.ColumnCount-1) do
  begin
    Application.ProcessMessages;
    
    sFieldName := tv.Columns[i].DataBinding.FieldName;
    with tv.DataController.DataSource.DataSet.FindField(sFieldName) do
    begin
      Tag := 1;
      DisplayLabel := tv.Columns[i].Caption;
      DisplayWidth := tv.Columns[i].DataBinding.Field.DisplayWidth;

//      if (tv.Columns[i].DropDownRows > 0) and
//         (tv.Columns[i].DropDownRows <> 7) then
//        DisplayWidth := tv.Columns[i].DropDownRows;
    end;
  end;

  if Assigned(Dep) then
    Result := kPrint( tv.DataController.DataSource.DataSet, Printer, Dep)
  else begin
    _Dep := TDeposito.Create;
    try
      _Dep.SetDeposito( 'TITULO', Titulo);
      _Dep.SetDeposito( 'MARGEM_DIREITA', 80);
      _Dep.SetDeposito( 'COMPRIMIDO', False);
      Result := kPrint( tv.DataController.DataSource.DataSet, Printer, _Dep);
    finally
      _Dep.Free;
    end;
  end;
