...
var
  ...
  i, iPos: Integer;
begin
//  if not kExistTable('SYSTEM') then
//  begin
//    kErro('Não foi encontrado a tabela system.');
//    Exit;
//  end;
  ...
  sWhere := 'WHERE LOCAL = :LOCAL AND CHAVE = :CHAVE AND ATIVO = TRUE';

  if  (kCountSQL('SELECT cast(COUNT(*) as integer) count FROM SYSTEM '+sWhere, [sLocal, sChave]) = 0) then
    Exit;
