...
Copyright (c) 2006, Allan Lima, Brasil.
...
uses
  ...
  {$IFDEF DBX}fbase_dbx,{$ENDIF}
  {$IFDEF IBX}fbase,{$ENDIF}
  Types, DB, fbase, DBClient;

type
  TFrmCadastro_dbx = class(TFrmBase_dbx)
...
procedure TFrmCadastro_dbx.FormCreate(Sender: TObject);
begin
  inherited;
  pvPesquisado    := False;
  pvImpresso      := False;
...
procedure TFrmCadastro_dbx.Iniciar;
var
  i: Integer;
begin
  ...
  kProcessaPesquisa( Self, mtListagem, '*', pvSELECT, pvSELECT_NULL);
...
procedure TFrmCadastro_dbx.lblSeparadorMouseDown(Sender: TObject;
...
procedure TFrmCadastro_dbx.dbgRegistroDblClick(Sender: TObject);
begin
  inherited;
  btnDetalhar.Click;
end;

procedure TFrmCadastro_dbx.btnNovoClick(Sender: TObject);
{var
...
procedure TFrmCadastro_dbx.dbgRegistroTitleBtnClick(Sender: TObject;
...
procedure TFrmCadastro_dbx.btnDetalharClick(Sender: TObject);
{var
  StateChange: TNotifyEvent;}
begin
  inherited;
...
procedure TFrmCadastro_dbx.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmCadastro_dbx.dtsRegistroStateChange(Sender: TObject);
var
  bEditando: Boolean;
begin
  ...
  if pvPesquisa then
    pnlPesquisa.Visible := not bEditando;

end;

procedure TFrmCadastro_dbx.FormResize(Sender: TObject);
begin
  inherited;
  WindowState := wsMaximized;
end;

procedure TFrmCadastro_dbx.FormClose(Sender: TObject;
...
procedure TFrmCadastro_dbx.FormShow(Sender: TObject);
begin
...
procedure TFrmCadastro_dbx.Pesquisar();
var
  sWhere: String;
begin
  ...
  sWhere := mtPesquisa.Fields[1].AsString;

  kAtualizaPesquisa( Self, mtListagem,
                     PesquisaValor.Text, pvSELECT, sWhere,
                     mtPesquisa.Fields[2].AsBoolean,
                     mtPesquisa.Fields[3].AsBoolean);

  pvPesquisado := True;

end;

procedure TFrmCadastro_dbx.mniOcultarClick(Sender: TObject);
begin
  inherited;
  mtListagem.Delete;
end;

procedure TFrmCadastro_dbx.btnImprimirClick(Sender: TObject);
var
  Dep: TDeposito;
begin
...
procedure TFrmCadastro_dbx.FrmBtnControlePadrao( Operacao: String);
var
  Wc: TWinControl;
...
begin
  ...
  begin   // Novo Registro
    ...
    if not pvDataSet.Active then
      if TClientDataSet(pvDataSet).ProviderName = '' then
        TClientDataSet(pvDataSet).CreateDataSet
      else
        TClientDataSet(pvDataSet).Open;
...
procedure TFrmCadastro_dbx.FrmDetalharClient;
var
  i: Integer;
  sField: String;
...
begin
  ...
  begin
    ...
    if (PesquisaValor.Text = '') then
      ...
      kAtualizaPesquisa( Self, mtListagem, PesquisaValor.Text, pvSELECT,
  ...
  begin
    ...
    if (mtListagem.RecordCount > 0) then begin

      for i := 0 to (mtListagem.FieldCount - 1) do begin
...
procedure TFrmCadastro_dbx.PageControl1Change(Sender: TObject);
begin
  RxTitulo.Caption := ' · '+TPageControl(Sender).ActivePage.Caption;
end;

function TFrmCadastro_dbx.GetDataSet: TDataSet;
begin
  if (PageControl1.ActivePage = TabListagem) then
    Result := mtListagem
  else
...
procedure TFrmCadastro_dbx.dbgRegistroDrawColumnCell(Sender: TObject;
...
{$IFDEF RX_LIB}
procedure TFrmCadastro_dbx.dbgRegistroGetCellParams(Sender: TObject;
...
procedure TFrmCadastro_dbx.dbgRegistroGetBtnParams(Sender: TObject;
...
procedure TFrmCadastro_dbx.dbgRegistroTitleClick(Column: TColumn);
begin
  inherited;
  kTitleClick( Column.Grid, Column, nil);
end;

constructor TFrmCadastro_dbx.Create(AOwner: TComponent);
begin
  inherited;
  {$IFDEF AK_LABEL}
